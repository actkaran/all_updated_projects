...
query = f"SELECT * FROM {db.db_link_table} WHERE status='pending'"
...
for row in rows:
    ...
    if response.status_code == 200:
        with open(f"{db.PAGESAVE}{hash_url}.html", 'w', encoding='utf-8') as f:
            f.write(response.text)
        db.cur.execute(f"UPDATE {db.db_link_table} SET status='save' WHERE hash_url=%s", (hash_url,))
        db.con.commit()
        print("page saved: ",hash_url)
        # time.sleep(2)
    else:
