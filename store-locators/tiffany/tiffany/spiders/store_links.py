...
from tiffany.items import TiffanyItem


def generate_hash_id(url):
    # Convert URL string into bytes
...
class StoreLinksSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url="https://www.tiffany.com/sitemap_Stores.xml",
                                 headers=h,
                                 callback=self.parse)
    ...
    def parse(self, response, **kwargs):
        ...
        for link in matches:
            item["url"] = link
            item["hash_url"] = generate_hash_id(link)
            item["sku"] = link.split("/")[-2]
            yield item


if __name__ == '__main__':
    execute(f"scrapy crawl {StoreLinksSpider.name}".split())
