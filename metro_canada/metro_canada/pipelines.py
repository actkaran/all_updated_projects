    ...
    def process_item(self, item, spider):
        if isinstance(item, MetroLinks):
            ...
            try:
                ...
                insert = f"INSERT INTO {db.db_link_table} (product_number, url) VALUES(%s, %s);"
                self.cur.execute(insert, (item["product_number"], item["url"]))
                self.con.commit()
                print("Inserted....")
        ...
        if isinstance(item, MetroCanadaItem):
            ...
            try:
                ...
                self.con.commit()
                print("Inserted....")
                update = f"""UPDATE {db.db_link_table} SET status='Done' WHERE product_number=%s"""
                self.cur.execute(update, (item["product_number"],))
                self.con.commit()
            except Exception as e:
                print(e)
